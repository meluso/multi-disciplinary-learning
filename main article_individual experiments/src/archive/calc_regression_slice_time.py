...
def normalize_df(df):
    ...
    list_of_lists = [get_graph_vars(), get_fn_vars('metrics')]
...
def run_regression(model, slice_name, mode, out_var, out_log=None):
    results = regress_points(model, slice_name, mode, out_var, out_log)
    return generate_table(model, slice_name, mode, results, out_var, out_log)


if __name__ == '__main__':
    
    reg_types = ['fixed_by_fn_type','fixed_by_fn_subtype']
    models = ['3xx','3xg']
    times = [5,10]
    out_vars = ['team_performance', 'team_productivity']
    out_log = None
    
    for reg, model, time, out in product(reg_types, models, times, out_vars):
        run_regression(model, f'time{time:02}', reg, out, out_log)
