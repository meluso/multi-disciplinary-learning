...
if __name__ == "__main__":
    ...
    index_degree = 1
    params = {
        'agent_fn': ['root','sphere','edgewt-root','edgewt-sphere'],
        'net_size': [1000],
        'net_type': ['power']
        }
    runs = 100
    ...
    for ii, pre in enumerate(prefixes):
        for jj, fn in enumerate(fns):
            line = net2fn2metric2results[pre + fn]['power']['avg_avg_uniform'] \
                - net2fn2metric2results[pre + fn]['power']['avg_avg_weighted']
            axs[ii,jj].plot(line)
            axs[ii,jj].set_title(pre + fn)
    fig.tight_layout()
    plt.show()
