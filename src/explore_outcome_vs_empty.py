...
outcomes = list(uv.get_outcomes().keys())
...
def plotter(time, file_in, file_out, activity, model, plot_opts,
    ...
    cols = (outcomes,['diff_mean','diff_ci_lo','diff_ci_hi'])
    ...
    for ii, (key, dct) in enumerate(zip(key_array, dct_array)):
        ...
        data = params2stats.loc[key,cols]
...
def build_outcomes_plot(df, dct):
    ...
    fig, axs = plt.subplots(
        len(outcomes), len(sizes), sharey='row', figsize=(15,12)
        )
    ...
    for ii, oc in enumerate(outcomes):
        ...
        for jj, sz in enumerate(sizes):
            ...
            for kk, gr in enumerate(graphs):
                ...
                mask = (df['team_size'] == sz) & (df['team_graph_type'] == gr)
                data = df[mask].loc[:,oc].join(df[mask]['run_step'])
                ...
                # Plot line
                axs[ii,jj].plot('run_step', 'diff_mean',
                                data=data, label=gr, color=colors[kk])
...
def plot_vs_empty(time, execset, models):
    ...
    for model in models:
        ...
        file_out = f'../figures/outcomes/outcome_vs_empty_{model}.pdf'
