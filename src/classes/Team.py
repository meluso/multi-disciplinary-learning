...
class Team(nx.Graph):
    ...
    def step(self):
        ...
        for ag in self:
            ...
            # Get fx and fx_new for this model
            fx = self.get_agent_fx(ag)
            fx_new = self.get_agent_fx_new(ag)
            ...
            # else keep the current x and fx values
    ...
    def gen_mc_samples(self):
        ...
        # Calculate team f(x), and integral while we're at it
    ...
    def calc_align_interdep(self, ag1, ag2):
        ...
        y1 = self.get_agent_mc_fx(ag1)
        y2 = self.get_agent_mc_fx(ag2)
    ...
    # No need to set team x's because provided by neighborhoods
    ...
    def get_nbhd_fn_diff_integral(self):
        '''Returns na because neighborhoods not applicable to this model.'''
        return nan
...
class TeamType3xx(Team):
    ...
    def __init__(self, team_size, team_graph_type, team_graph_opts, 
        ...
        # Inhert Team stuff but with additional inputs
        super().__init__(team_size, team_graph_type, team_graph_opts, 
                     team_fn_type, team_fn_opts, nbhd_fn_type, nbhd_fn_opts, 
                     agent_fn_type, agent_fn_opts, agent_steplim)
    ...
    def step(self):
        ...
        # Get new x and f(x) for each agent
        for ag in self:
            self.set_agent_x_new(ag)
            self.set_agent_fx_new(ag)
            
        # Get old g(x) and new g(x) for each agent
        for ag in self:
            self.set_nbhd_fx(ag,
    ...
    def store_fn_metrics(self):
        ...
        # Get store_fn_metrics from Team
        Team.store_fn_metrics(self)
        
        # Populate difficulty metrics for neighborhood as well
